...
(function(){
  ...
  helper.init = function(cb){
    $iframeContainer = $("<div id='iframe-container'></div>");
  ...
  helper.newDoc = function(cb, type){
    //build opts object
    ...
    if(!type)
      type = 1
    $iframe = $("<iframe src='/new/" + type + "'></iframe>");

    //clean up inner iframe references
    helper.padChrome$ = helper.padOuter$ = helper.padInner$ = helper.$ = null;
    ...
    $iframeContainer.find("iframe").purgeFrame().done(function(){
      ...
      $iframe.one('load', function(){
        helper.waitFor(function(){
          return !$iframe.contents().find("#nprogress").is(":visible") &&
            $iframe.contents().find(".content").is(":visible");
        }, 50000).done(function(){
          helper.$ = helper.padChrome$ = getFrameJQuery(                $('#iframe-container iframe'));
          //helper.padOuter$  = getFrameJQuery(helper.padChrome$('iframe[name="ace_outer"]'));
          //helper.padInner$  = getFrameJQuery( helper.padOuter$('iframe[name="ace_inner"]'));

          helper.$.fn.sendkeys = $.fn.sendkeys;
